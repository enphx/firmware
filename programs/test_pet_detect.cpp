...
void robotTask(void *arg) {
  ...
  robot.setArmPosition(3.2, 8.5, 60, false);
  ...
  while (1) {
    ...
    robot.scanAndGrabPet(SCAN_LEFT);
    robot.delay(1000);
    ...
    robot.scanAndGrabPet(SCAN_RIGHT);
    robot.delay(2000);
  ...
  robot.setArmPosition(0, 0, 8, true);
  ...
  // Move to pet 5 and set up arm for scanning.
  robot.resetDistanceTravelled();
  robot.setBaseSpeed(1.5);
  robot.armMoveSmooth(15, 8, 20, 300);
  robot.setArmPosition(15, 8, 60);
  while (robot.getDistanceTravelled() < 0.3) {
    robot.delay(1);
  }
  robot.armMoveSmooth(3.5, 8, 20, 500);
