...
void robotTask(void *arg) {
  ...
  robot.calibrateArm();
  robot.setArmPosition(4.3, 11, 0, false);
  robot.delay(5000);
  ...
  int32_t distanceToPet = robot.getClawDistance();
  while (distanceToPet > 200) {
    distanceToPet = robot.getClawDistance();
    robot.delay(1);
  }
  robot.delay(40);
  robot.setBaseSpeed(0);
  if (distanceToPet > 200 || distanceToPet < 0) {
    distanceToPet = 100;
  }

  robot.armMoveSmooth(3.5 + 1.5, 5.5 + distanceToPet / 25.4, 50, 300);
  robot.grabPet();
  robot.delay(500);
  robot.resetDistanceTravelled();
  robot.setBaseSpeed(1.5);
  ...
  while (robot.getDistanceTravelled() < 1) {
    robot.delay(1);
  }
  robot.armFollowTrajectory(scorePetOne, scorePetOneLength);
  robot.releasePet();
  robot.setArmPosition(8, 5, 0, false);
